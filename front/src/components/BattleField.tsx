...
export default function BattleField({
  ...
  return (
    <div className="relative w-full h-full bg-slate-950 overflow-hidden select-none font-sans">
      {/* --- Opponent Zone (Top/Red) --- */}
      {/* Outer Border Layer */}
      <div
        className="absolute inset-0 z-10 bg-red-500"
        style={{
          clipPath: clipPathOpponent,
          filter: "drop-shadow(0 0 10px rgba(239, 68, 68, 0.5))",
        }}
      >
        {/* Inner Content Layer */}
        <div
          className={`absolute inset-[2px] transition-colors duration-300 ${
            !isMyTurn && !winnerId ? "bg-red-900/40" : "bg-red-950/80"
          }`}
          style={{
            clipPath: clipPathOpponent,
            // Slight manual adjustment for perfect border thickness if needed, but inset-[2px] usually works well enough
          }}
        >
          <div className="absolute top-0 left-0 w-full h-[55%] flex flex-col justify-between p-4 pb-12">
            {/* Opponent Header */}
            <div className="flex justify-between items-start pl-8 pr-2 pt-1">
              <div className="flex flex-col">
                <h3 className="text-red-300 font-bold flex items-center gap-2 text-lg">
                ...
                {!isMyTurn && !winnerId && (
                  <span className="text-sm text-red-400 animate-pulse font-mono font-bold mt-1">
                    ◀ BATTLE PHASE
                  </span>
                )}
              </div>
            ...
            {/* Opponent Field Content */}
            <div className="flex-1 flex flex-col items-center justify-end gap-2 pr-[20%]">
              {" "}
              {/* Shift content left slightly away from cut */}
              {/* Bench */}
      ...
      {/* --- Player Zone (Bottom/Blue) --- */}
      {/* Outer Border Layer */}
      <div
        className="absolute inset-0 z-10 bg-cyan-500"
        style={{
          clipPath: clipPathPlayer,
      ...
      >
        ...
        <div
          className={`absolute inset-[2px] transition-colors duration-300 ${
            isMyTurn && !winnerId ? "bg-blue-900/40" : "bg-blue-950/80"
          }`}
          style={{
            clipPath: clipPathPlayer,
          }}
        >
          <div className="absolute bottom-0 left-0 w-full h-[55%] flex flex-col justify-between p-4 pt-12">
            {/* Player Field Content */}
            <div className="flex-1 flex flex-col items-center justify-start gap-4 pl-[20%]">
              {" "}
              {/* Shift content right slightly away from cut */}
              {/* Active */}
              <div className="relative z-20 transform -translate-y-4">
                {myActive ? (
                  <div
                  ...
                  <div className="absolute top-1/2 left-full ml-4 -translate-y-1/2 w-max">
                    <button
                      ...
                      className={`
                            ...
                            ${
                              !isMyTurn || !!winnerId
                                ? "opacity-0 pointer-events-none translate-x-10" // Hide when not turn
            ...
            <div className="flex justify-between items-end mt-2 pl-2 pr-8 pb-1">
              <div className="flex flex-col bg-slate-900/80 p-2 pr-6 rounded-tr-xl border-l-[3px] border-cyan-500 backdrop-blur-md">
                <h3 className="text-cyan-300 font-bold flex items-center gap-2 text-xl">
                ...
                {isMyTurn && !winnerId && (
                  <span className="text-sm text-cyan-400 animate-pulse font-mono font-bold mt-1">
                    ▶ YOUR TURN
                  </span>
                )}
              </div>
      ...
      <div
        ...
        style={{
          // Decorative gradient path following the gap
          background: `
      ...
      >
        ...
        {/* The Cut Line Visuals - A glowing line along the diagonal split */}
        {/* Top Diagonal: 40% 35% -> 60% 55% */}
        <div
          className="absolute"
          style={{
            top: "35%",
