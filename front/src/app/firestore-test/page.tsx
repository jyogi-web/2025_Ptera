...
export default function FirestoreTestPage() {
    ...
    const fetchCards = async () => {
        setStatus("Fetching...");
        setCards([]); // Clear current cards to verify fetch
        try {
            console.log("fetchCards: Calling getCards...");
            const data = await getCards();
            console.log("fetchCards: Success, count:", data.length);
            setCards(data);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            console.log("handleSubmit: Calling addCard...");
            await addCard({
                ...formData,
                grade: Number(formData.grade),
                creatorId: user.id,
            });
            console.log("handleSubmit: addCard finished.");
            alert("Added!");
            fetchCards();
        } catch (e) {
    ...
    return (
        <div className="flex min-h-screen flex-col items-center gap-8 p-8 bg-gray-50">
            ...
            <p className="text-sm text-gray-500">Connected to Project ID: <span className="font-mono font-bold text-black">{projectId}</span></p>

            {!user && <p className="text-red-500">Please login to add cards.</p>}

            <form onSubmit={handleSubmit} className="flex w-full max-w-md flex-col gap-3 rounded bg-white p-6 shadow">
            ...
            <div className="w-full max-w-2xl">
                ...
                <p className={`text-sm mb-4 ${status.includes("Error") ? "text-red-500 font-bold" : "text-blue-600"}`}>
                    Status: {status}
                </p>
                <div className="grid gap-4 sm:grid-cols-2">
                    {cards.map((card) => (
                        <div key={card.id} className="rounded border bg-white p-4 shadow-sm">
                            <h3 className="font-bold">{card.name} ({card.grade})</h3>
