...
export default function CirclePage() {
  ...
  const handleCreateCircle = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Reload page to reflect user's circleId or redirect?
      // Since AuthContext listens to auth state, we might need to manually refresh the user object or wait for it.
      // Ideally, createCircle updates Firestore, and we should update the local user state or force a reload.
      // For simplicity, let's force a reload for now or trust the next navigation/auth sync.
      // Actually, since we use onAuthStateChanged, it might not pick up the Firestore change immediately unless we refetch.
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white p-4 pt-20">
        <Animator active={true}>
          <div className="max-w-4xl mx-auto space-y-8 p-6 bg-gray-800/50 backdrop-blur-md rounded-xl border border-gray-700/50 shadow-2xl">
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-center p-4">
      {/* Background Ambience similar to Landing */}
